...
- macOS 使用 Keychain，Windows 使用 Credential Manager，Linux Desktop 使用 Secret Service。
- Headless Linux 的文件密钥存储必须为 `0600`，并在 `doctor` 中显示警告。
- daemon 使用 data directory 下的 `outbox.key`（`0600`）派生 AES-256-GCM，SQLite outbox payload 在落盘时加密；丢失该密钥时必须走备份恢复，不能静默生成新 key 解密旧消息。
